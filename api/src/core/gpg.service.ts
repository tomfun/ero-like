...
export class NoPublicKeyVerifyError extends Error {
  constructor(message: string, public readonly keyType, public readonly keyFingerprint) {
    super(message)
  }
}
...
export class GpgService {
  ...
  public async armor(input: string | Buffer): Promise<string | Buffer> {
    ...
    const getSignature = spawn(
      'gpg',
      ['--no-autostart', '--utf8-strings', isOutString ? '--enarmor' : '--dearmor'],
      {
  ...
  async temporaryImportAndVerify(importAndVerifyPayload: ImportAndVerifyPayload) {
    ...
    try {
      ...
      const e =
        err instanceof NoPublicKeyVerifyError ? new InvalidDataError(err.message) : err
      const log = `${e.message}: ${errs.join()}`
      if (e instanceof InvalidDataError) {
        this.logger.debug(log)
      } else {
  ...
  private async importCommon(
    ...
    out += errs.join('')
    const importMatchKey = out.match(/gpg: key ([0-9A-F]+): public key "(.+)" imported\n/)
    if (!importMatchKey) {
  ...
  private async verifyCommon(
    ...
    const verifyMatchMainKey = out.match(/Primary key fingerprint: ([0-9A-F ]+)/)
  ...
  private async signaturePacket(
    ...
    const getSignature = spawn(
      'gpg',
      ['--no-autostart', '--utf8-strings', '--lock-once', '-v', '--list-packets'],
      {
        cwd: gnuPgHome,
        stdio: ['pipe', 'pipe', 'pipe'],
        env: {
    ...
    const mainKeyV4 = getSignatureMatchMainKeyV4 ? getSignatureMatchMainKeyV4[1] : null
  ...
  private async getUserPublicKeys(user: string, gnuPgHome: string, errs: any[]) {
    errs.length = 0
    let out = ''
