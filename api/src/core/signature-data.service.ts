...
import { GpgService, InvalidDataError, NoPublicKeyVerifyError } from './gpg.service'
...
export class SignatureDataService {
  ...
  async userRegistration(importAndVerifyPayload: ImportAndVerifyPayload) {
    const now = new Date()
    const data = await this.gpgService.temporaryImportAndVerify(importAndVerifyPayload)
      ...
      if (publicKey.id) {
        if (keyData.expires && keyData.expires > now) {
          ...
          if (publicKey.invalidAt) {
            publicKey.invalidAt = publicKey.invalidAt < now ? publicKey.invalidAt : now
          } else {
    ...
    agreementSignatureBlock = await this.findOrDefaultBlock(agreementSignatureBlock)
    ...
    agreementSignature.primaryKeyFingerprint = signatureData.primaryKeyFingerprint
    ...
    agreementSignature.publicKey = publicKeys.find(
      (pk) => pk.publicKeyFingerprintString === signatureData.usedKeyFingerprint,
    )
  ...
  async createEntity(data: { clearSignArmored: string; type: string }) {
    const now = new Date()
    let verifyData: UnwrapPromise<ReturnType<typeof GpgService.prototype.verify>>
    ...
    try {
      ...
      if (!verifyData) {
        throw new InvalidDataError('verification failed: used unregistered key')
      }
    }
    ...
    dataEntity.sha256 = await this.calculateSha256(dataEntity.clearSignDataPart)
  ...
  private async findOrDefaultBlock(block: BlockEntity) {
    const b = await this.blockRepo.findOneBy(pick(block, ['blockArmored', 'type']))
    return b || block
  }
