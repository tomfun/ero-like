...
declare module 'vue' {
  interface ComponentCustomProperties {
    ...
    $d: (
      date: number | Date,
      args?: Record<string, string> | FormatJs.DateTimeFormatOptions,
    ) => FormatJs.DateTimeFormatResult
    $dx: (
...
export const createLocale = (options: FormatJsOptions): Vue.Plugin => ({
  install(app) {
    ...
    app.config.globalProperties.$dx = (
      d: number,
      args?: Record<string, string> | FormatJs.DateTimeFormatOptions,
    ) => app.config.globalProperties.$d(d * 1000, args)
