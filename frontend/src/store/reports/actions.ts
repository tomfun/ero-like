...
// };
const nothing = () => false
type BuildReportsFiltersScopedString<
  ScopedReport,
  K extends keyof ScopedReport,
  FieldType,
> = (
...
function buildReportsFiltersScopedString<ScopedReport, K extends keyof ScopedReport>(
  ...
  return fields.reduce(
    (filters: Array<(r: ScopedReport) => boolean>, field: K & string) => {
      ...
      if (field.includes('.*')) {
        ...
        if (endWithDot) {
          ...
          g = (r: ScopedReport) =>
            get(r, array)
              .map((e: Record<string, string | number>) => e[end])
              .join(' ')
        } else {
...
function buildReportsFilters(
...
): false | Array<(r: Report) => boolean> {
  const userFieldFilters = (
    buildReportsFiltersScopedString as BuildReportsFiltersScopedString<
  ...
  const dFieldFilters = (
    ...
    .call(
      this as Record<
        'd.title' | 'd.substances.*.namePsychonautWikiOrg',
        FilterRecordPair<string>
      >,
      'd.',
    ...
    .concat(
      (
        buildReportsFiltersScopedNumeric as BuildReportsFiltersScopedString<
          Report['d'],
          'dateTimestamp',
          number
        >
      ).call(this as Record<'d.dateTimestamp', FilterRecordPair<number>>, 'd.', [
        'dateTimestamp',
      ]),
    )
...
export default {
  async [FETCH_REPORTS](
    ...
    commit(SET_PAGINATION, {
      ...pagination,
      viewIds: localReportIds.slice(
        localIndexDesired,
        localIndexDesired + desiredPageSize,
      ),
    })
  ...
  async [FETCH_REPORT]({ commit }: ActionContext<unknown, unknown>, id: string) {
